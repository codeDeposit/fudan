package com.jy.common.utils.tree;
...
public class MenuTreeUtil {
	 ...
	 * 首页建立树菜单
	 */
	public static String indexBuildTreeHtml(List<Resources> menusDB){  
    	StringBuffer html = new StringBuffer();//定义菜单数组
        for (int i=0;i<menusDB.size();i++) { //得到菜单数
        	Resources node = menusDB.get(i);//菜单对象
            if ("0".equals(node.getParentId())) { 
        		html.append("<div id='menuBox'><a href='javascript:void(0)' onclick=\"getMenu("+node.getLayer()+",\'"+ node.getId() +"\')\" >");
        		html.append("<div class='menuIcon'><i class='"+ node.getIcon() +"'></i></div><div id='menuName'>");
	 ...
	 * 首页建立树菜单
	 */
	public static String mobileIndexBuildTreeHtml(List<Resources> menusDB){  
    	StringBuffer html = new StringBuffer();//定义菜单数组
        for (int i=0;i<menusDB.size();i++) { //得到菜单数
        	Resources node = menusDB.get(i);//菜单对象
            if ("0".equals(node.getParentId())) { 
        		html.append("<div class='aui-col-xs-6' onclick=\"getMenu("+node.getLayer()+",\'"+ node.getId() +"\','"+ node.getName() +"')\" >");
        		html.append("<i class='"+ node.getIcon() +"' style='font-size:32px;'></i>");
        		html.append("<div class='aui-grid-label'>");
				html.append(node.getName()); 	
				html.append("</div></div>"); 	
            }  
        }  
