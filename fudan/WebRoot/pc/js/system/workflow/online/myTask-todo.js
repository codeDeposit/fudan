    		 ...
    		 if(results!=null&&results.length>0){
        		 ...
        		 for(var i = 0;i<results.length;i++){
            		 ...
            		 /*if('deptAudit'==taskDKey||'hrAudit'==taskDKey){*/
            			 html+="<td class='left'><button class='btn btn-xs btn-success' onclick='todoTask(&apos;"+l.id+"&apos;,&apos;"+l.processInstanceId+"&apos;)' ><i class='icon-pencil align-top bigger-125'></i>同意</button></td>";
            		 /*}else{
	...
	JY.Ajax.doRequest(null,jypath +'/backstage/workflow/online/myTask/findTask',{pId:pId},function(data){
		...
		JY.Model.audit("auDiv","审核"
		 ,function(){	
			//同意申请
			$(this).dialog("close");
			JY.Model.tankuang("test1","下个审核人选择",function(){	
				var approver = $('#employeetable').find('input[type="checkbox"]').parent().parent().children('td').eq(2).attr("id");
				 if(JY.Object.notNull(approver)){
					 var that =$(this);
					 JY.Ajax.doRequest("",jypath +'/backstage/workflow/online/myTask/complete/'+id,{"approver":approver,"input":1},function(data){
						 JY.Model.info(data.resMsg,function(){search();that.dialog("close");});
					 }); 
				 }else{
			    	$("#employeetable").tips({side:1,msg:"请选择审核人！",bg:'#FF2D2D',time:1});
			     }
			});
		},function(){
			//驳回申请
			...
			JY.Model.edit("rejectDiv","驳回",function(){	
				 ...
				 if(JY.Object.notNull(rejectReason)){
					 ...
					 JY.Model.confirm("确认驳回吗？",function(){			 
						 ...
						 ovar.pId=pId;*/
						 JY.Ajax.doRequest("",jypath +'/backstage/workflow/online/myTask/reject/'+id,{"input":2},function(data){
							 JY.Model.info(data.resMsg,function(){search();that.dialog("close");});
						 });	
						});     
				 }else{
