/**
 * 方法：
 * 	  	成功状态：		fuSuccess('文字');				//默认显示："操作成功!"
 * 		失败状态：		fuError('文字');					//默认显示："操作失败!"
 * 		选择按钮弹出框：	fuConfirm('文字'); 				//默认显示："确定执行此操作吗?",参数：param1(参数变量)，param2(参数变量),param3(文字)
 * 						确定按钮对应的方法：fuSure();		//在fuSure()方法中编写自己的代码
 * 		定时消失弹出框：	1.成功：fuShowSuccess('文字');	//默认显示："操作成功!" 
 * 						2.失败：fuShowError('文字');		//默认显示："操作失败!"
 * 		加载状态框： 	fuLoading();
 * 		关闭弹出框：		fuClose();
 * 		关闭加载动画     fuCloseLayer()
 */
...
//关闭加载动画
function fuCloseLayer(){
	document.getElementById('toastLayerClose').style.display = "none";
}
//加载状态，fuCloseLayer();为关闭方法
function fuLoading(){
	document.getElementById('toastLayerClose').style.display = "block";
}
//成功状态
function fuSuccess(param){
	var lWord = "";
	...
	swal({
		...
		text: lWord,
		type: "success",
		confirmButtonColor: "#15bc75",
		confirmButtonText: "确定",
		allowOutsideClick:true
	});
}
//失败状态
function fuError(param){
	var lWord = "";
	if(param == undefined || param == ''){
		lWord = "操作失败!";
	}else{
...
//选择弹出框，确定对应fuSure()方法
function fuConfirm(param1,param2,param3){
	var lWord = "";
	...
	},function(data){//返回值true/false
		if(data){
			fuSure(param1,param2);//确定方法
		}
	});
}
