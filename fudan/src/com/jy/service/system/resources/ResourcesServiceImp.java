...
public class ResourcesServiceImp extends BaseServiceImp<Resources> implements ResourcesService {
	 ...
	 * 得到二级菜单
	 */
	public List<Resources> selectChildMenuTree(String menuId){
		ResourcesDao dao = ((ResourcesDao) baseDao);
		return dao.selectChildMenuTree(menuId);
	}
	/*
	 * 获取所有有权限的菜单，菜单的父菜单为0
	 */
	@Override
	public List<Resources> findList(Account acount) {
		ResourcesDao dao=(ResourcesDao) baseDao;
		List<Resources> menus = dao.selectList(acount.getRoleId());
		return menus;
	}
	@Override
	public List<Resources> appMenuTree(String roleId, String layer) {
		return ((ResourcesDao) baseDao).appMenuTree(roleId,layer);
	}
